...
class EdgeGCNLayer(nn.Module):
    ...
    def forward(self, g, node_feats, edge_feats):
        ...
        with g.local_scope():
            ...
            u, v = g.edges()
            h_e = torch.cat((g.srcdata['h'][u], g.dstdata['h'][v]), 2)

            return rst, h_e # il y a un bootleneck car la taille des node embeddings reste juste 39! contre 256 dans sage


class EdgeGCN2(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, norm, bias, residual, last_encoder_activation, aggreg, sample_neigh=False):
        super(EdgeGCN2, self).__init__()
        self.gcn1 = EdgeGCNLayer(in_feats, in_feats*1, norm=norm, bias=bias, aggreg=aggreg)
        self.gcn2 = EdgeGCNLayer(in_feats*2, in_feats*4, norm=norm, bias=bias, aggreg=aggreg)
        self.W_edge = nn.Linear(in_feats*8, 256)
    ...
    def forward_full_neigh(self, g, node_feats, edge_feats, corrupt=False):
        ...
        # add drop edge
        h, h_e = self.gcn1(g, node_feats, edge_feats)
        h = F.relu(h)
        # add dropout
        h, h_e = self.gcn2(g, h, h_e)

        if self.last_encoder_activation == "relu":
            h = F.relu(h)
        elif self.last_encoder_activation == "tanh":
            h = F.tanh(h)
