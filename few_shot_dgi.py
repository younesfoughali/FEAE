...
from viz import get_xs_ys_to_plot
...
parser.add_argument('--encoder-patience', type=int, default=150)
...
parser.add_argument('--use-hybrid-ssl-loss', type=int, default=1)
...
for file_name in datasets:
    ...
    for it in range(args.nb_iter):
        ...
        xs, ys = get_xs_ys_to_plot(dgi, model_to_viz, pos_augmentation, neg_augmentation, train_g, DEVICE, few_shot_mal_indices, other_attack_indices)

        annotations = ["positive", "negative", "few-shot", "attack"]
        for label, (x, y) in enumerate(zip(xs, ys)):
            ax.scatter(x, y, c=np.array(cmap(2**label)).reshape(1,4), label=annotations[label], alpha=0.5)
        ax.legend(fontsize='large', markerscale=2)

        svg_path = f'{log_prefix}/pos_neg_embeds.svg'
        plt.axis('off')
        ...
        report = log_regression(few_shot_emb, testing_emb, few_shot_labels, test_labels, \
            log_path=log_prefix, epochs=args.mlp_epochs, patience=args.mlp_patience, mode="few-shot", logger=log, device=device, iterations=args.mlp_nb_iter, lr=args.mlp_lr)
        few_shot_best_metrics.add_report(report)
        log("RESULTS for MLP few-shot:")
        log(report)

        #%%
        # Supervised MLP on all data
        report = log_regression(training_emb, testing_emb, train_labels, test_labels, \
            log_path=log_prefix, epochs=args.mlp_epochs, patience=args.mlp_patience, mode="supervised", logger=log, device=device, iterations=args.mlp_nb_iter, lr=args.mlp_lr)
        supervised_best_metrics.add_report(report)
        log("RESULTS for MLP supervised:")
        log(report)

        #%%
        # IF Detection
        # report = isolation_forest(benign_train_samples, test_samples, test_labels.detach().cpu())
        # few_shot_best_metrics.add_report(report)
        # log("RESULTS for IF:")
        # log(report)
    log("")
